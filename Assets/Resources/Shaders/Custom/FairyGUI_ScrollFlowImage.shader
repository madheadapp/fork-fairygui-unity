Shader "FairyGUI/ScrollFlowImage"
{
     Properties
    {
        _MainTex ("Base (RGB), Alpha (A)", 2D) = "black" {}
        _FlowTex ("Noise (RGB), Alpha (A)", 2D) = "white" {}
	    ...
	    _ScrollSpeed("Scroll Speed MainTex (x,y) FlowTex (z,w)", Vector) = (0,0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // Main Tex Scrolling
                float2 mainTexSpeed = float2(_ScrollSpeed.x , _ScrollSpeed.y);
                float2 uv0_MainTex = 1.0 * _Time.y * mainTexSpeed + i.texcoord.xy; 
                
                 // Flow Map Scrolling
                float2 distortSpeed = float2(_ScrollSpeed.z , _ScrollSpeed.w);
                float2 uv0_FlowTex = ( 1.0 * _Time.y * distortSpeed + i.texcoord.xy * _FlowTex_ST.xy + _FlowTex_ST.zw);

                fixed4 col = tex2D( _MainTex, ( uv0_MainTex - ( (( tex2D( _FlowTex, uv0_FlowTex ))).rg * _FlowPower ) ) ) * i.color;
