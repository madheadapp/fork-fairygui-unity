...
Shader "FairyGUI/VisualNovelCharacterFlashImage"
{
    Properties
    {
        ...
        [Toggle] _IsShine("IsShine", Int) = 0  
        
        [Toggle] _UseRgbAsAlpha("Use Rgb As Alpha", Int) = 0  
        
		//shine variables
		_PrimaryDirectionX ("PrimaryDirectionX", Float) = -1500
		_PrimaryDirectionY ("PrimaryDirectionY", Float) = -1000
		_PrimaryFrequency ("Primary Light Frequency", Float) = 15
		_PrimaryWaveLength ("Primary Light WaveLength", Float) = 1.5
		_PrimaryLightLocation("Primary Light Location", Float) = 0.92
        ...
        Pass
        {
            CGPROGRAM
                ...
                fixed4 frag (v2f i) : SV_Target
                {
                    ...
                    fixed4 col = tex2D(_MainTex, i.texcoord.xy / i.texcoord.w) * i.color * fixed4(_Color.rgb, 1);
                    //col.a *= tex2D(_MainTex, i.texcoordAlpha.xy / i.texcoordAlpha.w).r;
                    ...
                    float x = sin( _CustomTime *  _PrimaryFrequency + i.vertex.x / _PrimaryDirectionX + i.vertex.y / _PrimaryDirectionY) * _PrimaryWaveLength - _PrimaryLightLocation;
					fixed3 shineOffset = fixed3(1,1,1) * clamp(x, 0, 1);
                        
                    return fixed4(col.rgb * (1 - _MultiplyFactor * _IsDarkened) + shineOffset, alphaMultiplier * col.a);
                }
            ENDCG
        }
    }
