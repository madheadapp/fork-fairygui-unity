Shader "FairyGUI/GlitchEffect"
{
    Properties
    {
        _MainTex ("Base (RGB), Alpha (A)", 2D) = "black" {}
        _ScanLineJitter ("Jitter", vector) = (0,0,0,0)
        _VerticalJump ("Vertical Jump", vector) = (0,0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                fixed4 src1 = tex2D(_MainTex, frac(float2(i.texcoord.x + jitter + shake, jump)));
                fixed4 src2 = tex2D(_MainTex, frac(float2(i.texcoord.x + jitter + shake + drift, jump)));
                col = fixed4(src1.r, src2.g, src1.b, src1.a);
