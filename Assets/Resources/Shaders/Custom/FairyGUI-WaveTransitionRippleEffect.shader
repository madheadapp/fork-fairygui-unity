Shader "FairyGUI/WaveTransitionRippleEffect"
{
    Properties
    {
    	...
    	[Space]
		_CenterPositionX( "Center Position X", Float ) = 0
    	_CenterPositionY( "Center Position Y", Float ) = 0
    	_Radius ("Radius", Float) = 0
    	_Feather( "Feather( Larger -> Harder )", Float ) = 1000
    ...
    SubShader
    {
		...
		pass
    	{
			...
			fixed4 frag(v2f i) : COLOR{
				...
				float currentDistance = distance( float2(_CenterPositionX, _CenterPositionY), i.worldPos.xy );
				if( currentDistance < _Radius )
				{
					col.a = lerp( col.a, 0, clamp( (_Radius - currentDistance)/_Feather, 0, 1 ) );
				}
				else if( currentDistance > _Radius + _Thickness )
				{
					col.a = lerp( col.a, 0, clamp( (currentDistance - _Radius - _Thickness)/_Feather, 0, 1 ) );
				}
