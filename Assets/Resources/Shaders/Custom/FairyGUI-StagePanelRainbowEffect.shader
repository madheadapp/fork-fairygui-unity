Shader "FairyGUI/StagePanelRainbowEffect"
{
     Properties
    {
		...
		_PrimaryWaveLength ("Primary Light WaveLength", Float) = 4
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float lightIntensity(float x, float directionOffset, float frequency, float clampValue, float waveLength, float density, float phaseShift )
			{
			    float location = x * frequency + directionOffset;
                return smoothstep(clampValue, 1, sin( location * waveLength + phaseShift ) * 0.5 + 0.5) * density;
			}
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float2 normalizedDirection = normalize(float2(_PrimaryDirectionX, _PrimaryDirectionY));
                float directionOffset = i.texcoord.x * normalizedDirection.x + i.texcoord.y * normalizedDirection.y;
                //float directionOffset = i.texcoord.x * dir.x + i.texcoord.y * dir.y;
                ...
                float primaryDensity = col.r * lightIntensity( _Time.y * _PrimaryLightLocation, directionOffset, _PrimaryFrequency, _PrimaryClampValue, _PrimaryWaveLength, _PrimaryDensity, 0) ;
                ...
                col.rgb += (float3(0.5, 0.5, 0.5) + 0.5 * cos( _Time.w + i.texcoord.xyx * 7 + float3(0,2,4)) + float3(0, -0.1, 0.1) )  * primaryDensity;
